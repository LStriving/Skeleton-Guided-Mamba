...
class ClsHead(nn.Module):
    ...
    def __init__(self,
        ...
        # classifier
        self.cls_head = MaskedConv1D(feat_dim,
                                     num_classes,
                                     kernel_size,
                                     stride=1,
                                     padding=kernel_size // 2)
        ...
        if len(empty_cls) > 0:
            bias_value = -(math.log((1 - 1e-6) / 1e-6))
            for idx in empty_cls:
                torch.nn.init.constant_(self.cls_head.conv.bias[idx],
                                        bias_value)
...
class RegHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.offset_head = MaskedConv1D(feat_dim,
                                        2 * (num_bins + 1),
                                        kernel_size,
                                        stride=1,
                                        padding=kernel_size // 2)
...
class PtTransformer(nn.Module):
    ...
    def __init__(
        ...
        *args, **kwargs        # other arguments
        ...
        # try to get SGP
        sgp_mlp_dim = kwargs.get('sgp_mlp_dim', 768)
        downsample_type = kwargs.get('downsample_type', 'max')
        k = kwargs.get('k', 5)
        init_conv_vars = kwargs.get('init_conv_vars', 0)
        self.sgp_win_size = kwargs.get('sgp_win_size', 1)
    ...
    def preprocessing(self, video_list, padding_val=0.0):
        ...
        if self.training:
            # print(max_len,self.max_seq_len)
...
'''
https://github.com/dingfengshi/TriDet.git
'''
