...
class Mamba(nn.Module):
    def __init__(
        ...
        if pool_method.startswith('2d-'):
            assert math.sqrt(pool_size) == int(math.sqrt(pool_size)),\
        ...
        if self.pool_method == "avg":
            ...
            self.pool = nn.MaxPool1d(self.pool_size, stride=self.pool_size, padding=0)
        elif self.pool_method == 'conv':
            kernel_size = self.pool_size + 1
            padding = kernel_size // 2
            self.pool = nn.Conv1d(self.d_inner * 2 * 2, self.d_inner * 2 * 2, bias=False,
                kernel_size=kernel_size, stride=self.pool_size, padding=padding, groups=self.d_inner * 2 * 2)
        elif self.pool_method == '2d-avg' or self.pool_method == '2d_avg':
            pool_size = (self.pool_size, self.pool_size)
            pool = nn.AvgPool2d(pool_size, stride=pool_size, padding=0)
            self.pool = partial(self._2d_pool, pool=pool)
        elif self.pool_method == '2d-max' or self.pool_method == '2d_max':
            pool_size = (self.pool_size, self.pool_size)
            pool = nn.MaxPool2d(pool_size, stride=pool_size, padding=0)
            self.pool = partial(self._2d_pool, pool=pool)
        elif self.pool_method == '2d-conv' or self.pool_method == '2d_conv':
            kernel_size = (self.pool_size + 1, self.pool_size + 1)
            padding = (kernel_size // 2, kernel_size // 2)
            pool = nn.Conv2d(self.d_inner * 2 * 2, self.d_inner * 2 * 2, bias=False,
                kernel_size=kernel_size, stride=(self.pool_size, self.pool_size), padding=padding, groups=self.d_inner * 2 * 2)
            self.pool = partial(self._2d_pool, pool=pool)
        else:
